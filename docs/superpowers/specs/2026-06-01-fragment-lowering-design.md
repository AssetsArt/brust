...
### Emit — factory (`emit_factory.rs`)

A fragment can reach the factory path as a child of an SSR component
(`<Layout><>…</></Layout>`), because `lower_ssr_component` lowers its children
through the shared `lower_child` (`lower.rs:701`). Two functions need arms:

- `collect_factories` — recurse into `Fragment.children`.
- `emit_child` — emit `h(Fragment, null, c1, c2, …)`.

`emit_child` is an exhaustive match (no `_ =>`), so adding `Fragment` without an
arm is a compile error — but a fragment IS a reachable child here, so the arm
must do the right thing (emit `h(Fragment, null, …)`), not no-op like the
`Document => {}` arm or `unreachable!()` like `Cond`/`ChildrenSlot`.
The factory module is generated by `native-routes-emit.ts`, which emits
`import { createElement as h } from 'react'`. `Fragment` must be imported there
too **when used**. To stay lint-safe (no unused import), surface a per-factory
`uses_fragment: bool` on `FactoryOutput` (mirroring the existing `uses_island`),
propagate it to `ComponentMeta`, expose it through the compile output JSON, and
have `native-routes-emit.ts` add `Fragment` to the import only when any factory
sets it.

> BLOCKED fallback (factory path): if the `uses_fragment` plumbing through the
> JSON/TS boundary proves larger than budgeted, reject a fragment that is a
> direct child of an SSR component with a new dedicated
> `ErrorKind::FragmentInSsrComponentNotSupported`. **Wire the rejection in
> `lower_ssr_component`'s own child loop** (`lower.rs:700–704`): after lowering
> each child via `lower_child`, check `matches!(node, JsxNode::Fragment { .. })`
> and error. This needs no SSR-context flag on `lower_child` (the generic path
> stays fragment-friendly for the jinja side). Keep the jinja path (root +
> host-element children + inline) fully supported. The jinja path is the primary
> goal; the factory path is secondary.
...
emit_factory::emit_child → h(Fragment, null, …)             (SSR-component child)
```
...
8. **`emit_factory.rs`** — a `Fragment` child of an SSR component emits
   `h(Fragment, null, …)` and sets `uses_fragment` (or, if the BLOCKED fallback
   is taken, the lower-time rejection test instead).
