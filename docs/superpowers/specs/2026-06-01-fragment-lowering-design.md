...
`emit_child` currently `unreachable!()`s on `Cond`/`ChildrenSlot`, so leaving
`Fragment` unhandled there would `panic` on a reachable input — unacceptable.
...
> JSON/TS boundary proves larger than budgeted, fall back to rejecting a fragment
> that is a direct child of an SSR component at lower time with a new dedicated
> `ErrorKind::FragmentInSsrComponentNotSupported`, and keep the jinja path
> (root + host-element children + inline) fully supported. The jinja path is the
> primary goal; the factory path is secondary.
...
- `lower.rs::splice_children_slots` — recurse into children AND splice any
  top-level `ChildrenSlot` entries in the fragment's own children vec (same
  in-place splice loop used for `Element.children` and `Document.body`)
