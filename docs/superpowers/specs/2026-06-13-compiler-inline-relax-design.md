...
Change:
- For entries with `tag == "style"` only: accept the same expression subset as head attrs (member paths via the existing path parser). Representation: `text` becomes `HeadValue` (today it is `Option<String>`); literal behavior byte-identical.
...
Lowering: for `MAP = {hero: Hero, gallery: Gallery}` and dispatch expr `k` with call-site props P:
```
{% if (k) == 'hero' %}   <inline of Hero with props P>
{% elif (k) == 'gallery' %}<inline of Gallery with props P>
{% endif %}
```
IR: nested `JsxNode::Cond` chain (test = equality `Expr::BinEq(path, StrLit)` — reuse the existing inline binary-op IR; alternate of the last = empty). Each branch inlines via the existing `try_native_inline` machinery (circular-inline guard applies per branch). If ANY mapped component is not inlinable → the whole dispatch falls back like today's non-inlinable case (warning, SSR fallback) — no partial native dispatch.
