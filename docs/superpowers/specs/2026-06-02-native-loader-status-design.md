...
- **Document render branch** (`makeRenderer` native path, ~599): the `isNativeVerdict`
  check is inserted **immediately after `data = await leaf.loader(...)` (~605–617),
  before the `JSON.stringify(data)` at ~617** (symbols are dropped by stringify, so the
  verdict must be intercepted first). `if (isNativeVerdict(data))`:
  ...
  - Normal return → `renderStatus = undefined` → `napiRenderJinja(...)` called with no
    status (Rust defaults to 200). **Both** `napiRenderJinja` call sites must thread
    `renderStatus`: the manifest branch (~654) and the plain branch (~682). Patching only
    one is a silent bug.
...
| `example/pokedex/lib/loaders.ts` | dogfood: detail 404 path wraps its return as `return notFound(emptyDetail(name))`. **Keep** the `notFound: boolean` field on `DetailData` (`lib/types.ts`) and the `notFound: false` on the success return — `DetailPage.tsx` still branches on the flag (S11 conditional); the sentinel sets HTTP status, the flag drives template branching. Do **not** remove the flag. |
...
- SPA navigation to a `notFound`/`redirect` route does a **full reload** (not in-place
  swap). Correct status, slightly less smooth. `redirect` specifically costs **2 round
  trips** on the nav path (full reload to the original URL → that load re-hits the verdict
  → document path emits the 3xx → browser follows it). Future: in-place handling in
  `bootstrap.ts`.
...
- **Focused Rust test for the status param is NOT feasible** without a live worker/SAB
  (`napi_render_jinja` needs a registered pool entry + pinned `BufPtr`, lib.rs ~842).
  Resolved: status param is covered by **TS integration only**; Rust unit tests cover
  `classify_request_body` (S12).
