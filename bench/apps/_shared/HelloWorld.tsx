...
export default function HelloWorld({ workerId }: RouteContext) {
  return (
    <Layout title="Home">
      ...
      <p>
        Rendered on <code>{`worker_id=${workerId ?? 0}`}</code> and shipped as a single chunk with{' '}
        <code>Content-Length</code> (no streaming needed — no <code>&lt;Suspense&gt;</code> in this
        tree).
      </p>
      <p>
        The counter below is a client-hydrated island: only that subtree boots React in the browser.
        Everything else stays static HTML.
      </p>
