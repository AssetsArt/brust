...
export function makeRespond() {
  return (body: unknown, init?: { status?: number; headers?: Record<string, string> }): ActionResponseSentinel => ({
    [RESPOND]: true, status: init?.status ?? 200, body, headers: init?.headers,
  })
}
...
type ParamKeys<P extends string> =
  P extends `${string}{${infer K}}${infer Rest}` ? (K extends `*${infer C}` ? C : K) | ParamKeys<Rest> : never
type Params<P extends string> = [ParamKeys<P>] extends [never] ? Record<string, string> : { [K in ParamKeys<P>]: string }
...
export interface ActionsBuilder<Acc extends EndpointMap = {}> {
  ...
  get<P extends string, O extends EndpointOptions, R>(path: P, handler: Handler<BodyOf<O>, Params<P>, QueryOf<O>, R>, opts?: O): ActionsBuilder<Acc & { [K in P]: { GET: { input: QueryOf<O>; output: Awaited<R> } } }>
  post<P extends string, O extends EndpointOptions, R>(path: P, handler: Handler<BodyOf<O>, Params<P>, QueryOf<O>, R>, opts?: O): ActionsBuilder<Acc & { [K in P]: { POST: { input: BodyOf<O>; output: Awaited<R> } } }>
  put<P extends string, O extends EndpointOptions, R>(path: P, handler: Handler<BodyOf<O>, Params<P>, QueryOf<O>, R>, opts?: O): ActionsBuilder<Acc & { [K in P]: { PUT: { input: BodyOf<O>; output: Awaited<R> } } }>
  patch<P extends string, O extends EndpointOptions, R>(path: P, handler: Handler<BodyOf<O>, Params<P>, QueryOf<O>, R>, opts?: O): ActionsBuilder<Acc & { [K in P]: { PATCH: { input: BodyOf<O>; output: Awaited<R> } } }>
  delete<P extends string, O extends EndpointOptions, R>(path: P, handler: Handler<BodyOf<O>, Params<P>, QueryOf<O>, R>, opts?: O): ActionsBuilder<Acc & { [K in P]: { DELETE: { input: BodyOf<O>; output: Awaited<R> } } }>
}
...
export function defineActions(): ActionsBuilder {
  ...
  function add(method: EndpointDef['method'], path: string, handler: (c: ActionContext) => unknown, opts?: EndpointOptions) {
    if (!isValidEndpointPath(path)) throw new Error(`defineActions: invalid endpoint path ${JSON.stringify(path)} (must start with '/', no whitespace/?#)`)
    const key = `${method} ${path}`
    if (seen.has(key)) throw new Error(`defineActions: duplicate endpoint ${key}`)
    seen.add(key)
    endpoints.push({ method, path, handler, body: opts?.body, query: opts?.query, middleware: [...globalMw, ...(opts?.middleware ?? [])] })
  }
  const builder = {
    endpoints,
