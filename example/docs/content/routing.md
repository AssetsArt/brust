...
| `cache` | `{ ttl_seconds, prefix?, bypass?, key?, key_ttl_seconds? }` | Opt-in two-layer page cache (declarative L1 + programmatic L2). Read from the **leaf only**. Works on **native** routes too — a native L1 hit is served from Rust with zero worker dispatch. See [Caching](/docs/caching). |
